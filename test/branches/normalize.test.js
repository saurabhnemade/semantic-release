...
test('Release branches - initial state', t => {
  ...
  t.deepEqual(
    ...
    [
      ...
      {type: 'release', name: 'next', range: '>=1.1.0 <2.0.0', accept: ['minor', 'patch'], channel: 'next'},
      {
        type: 'release',
        name: 'next-major',
        range: '>=2.0.0',
...
test('Release branches - 3 release branches', t => {
  ...
  t.deepEqual(
    ...
    [
      ...
      {type: 'release', name: 'next', range: '>=1.2.0 <2.0.0', accept: ['minor', 'patch'], channel: 'next'},
      {
        type: 'release',
        name: 'next-major',
        range: '>=2.1.0',
...
test('Release branches - 2 release branches', t => {
  ...
  t.deepEqual(
    normalize.release({release}).map(({type, name, range, accept, channel}) => ({type, name, range, accept, channel})),
    [
      {type: 'release', name: 'master', range: '>=1.2.0 <2.0.0', accept: ['minor', 'patch'], channel: undefined},
      {type: 'release', name: 'next', range: '>=2.1.0', accept: ['major', 'minor', 'patch'], channel: 'next'},
    ]
  );
});
...
test('Release branches - 1 release branches', t => {
  ...
  t.deepEqual(
    normalize.release({release}).map(({type, name, range, accept, channel}) => ({type, name, range, accept, channel})),
    [{type: 'release', name: 'master', range: '>=1.2.0', accept: ['major', 'minor', 'patch'], channel: undefined}]
  );
});
...
test('Release branches - Handle missing previous tags in branch history', t => {
  ...
  t.deepEqual(
    normalize.release({release}).map(({type, name, range, accept, channel}) => ({type, name, range, accept, channel})),
    [
      {type: 'release', name: 'master', range: '>=2.0.0 <3.0.0', accept: ['minor', 'patch'], channel: undefined},
      {type: 'release', name: 'next', range: '>=3.0.0', accept: ['major', 'minor', 'patch'], channel: 'next'},
    ]
  );
});
...
test('Release branches - enforce release gaps after downstream merge', t => {
  ...
  t.deepEqual(
    ...
    [
      {type: 'release', name: 'master', range: '>=2.0.0 <2.1.0', accept: ['patch'], channel: undefined},
      {type: 'release', name: 'next', range: '>=2.1.0 <3.0.0', accept: ['minor', 'patch'], channel: 'next'},
      {
        type: 'release',
        name: 'next-major',
        range: '>=3.0.0',
...
test('Release branches - limit releases on 2nd and 3rd branche based on 1st branch last release', t => {
  ...
  t.deepEqual(
    ...
    [
      ...
      {type: 'release', name: 'next', range: '>=3.1.0 <4.0.0', accept: ['minor', 'patch'], channel: 'next'},
      {
        type: 'release',
        name: 'next-major',
        range: '>=4.0.0',
